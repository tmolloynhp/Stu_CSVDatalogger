...
public class Stu_DataLoggerExporter : BaseNetLogic
{
    [ExportMethod]
    public void Export()
    {
    ...
    private string GetCSVFilePath()
    {
        ...
        string fmt = "00";
        string filedate = localTime.Year.ToString()+localTime.Month.ToString(fmt)+localTime.Day.ToString(fmt)+localTime.Hour.ToString(fmt) +localTime.Minute.ToString(fmt)+ localTime.Second.ToString(fmt);
        ...
        csvPathVariable = csvPathVariable + filedate + " log.csv";
        return new ResourceUri(csvPathVariable).Uri;
    }
    ...
    private string CreateQuery(Table table)
    {
        ...
        return $"SELECT {queryColumns} FROM \"{table.BrowseName}\" ORDER BY \"LocalTimestamp\" DESC LIMIT 1";
